...
export const ProblemForm: React.FC<ProblemFormProps> = ({ onSuccess }) => {
    ...
    const runBackgroundProcessing = async () => {
      ...
      try {
        ...
        setTranslationStatus({
          ...
          error: 'Translation failed; original text will be forwarded.'
        });
      }
      ...
      try {
        ...
        const response = await fetch(
          `${configuredApiBase}/citizen/problems/preview`,
          {
            method: 'POST',
            headers: {
        ...
        if (!response.ok || !data?.jobId) {
          throw new Error(
            data?.message || 'AI preview could not be queued'
          );
        }
        ...
        for (let attempt = 0; attempt < 60; attempt += 1) {
          ...
          const statusResponse = await fetch(
            `${configuredApiBase}/citizen/problems/jobs/${encodeURIComponent(data.jobId)}`
          );
          ...
          if (!statusResponse.ok) {
            throw new Error(
              status?.message || 'Could not read AI preview status'
            );
          }

          if (status.state === 'completed' && status.result) {
            setAiCompletionPrompt(status.result as AiPipelineResult);
            return;
          }

          if (status.state === 'failed') {
            throw new Error(status.error || 'AI preview failed');
          }
        }

        throw new Error(
          'AI preview is still running. You can continue manually.'
        );
      } catch (err: any) {
        console.warn('Background AI preview warning:', err);

        setSubmitError({
          title: 'AI preview notice',
          message:
            err?.message ||
            'AI preview is unavailable. You can continue manually.'
        });
      }
    };
  ...
  const handleConfirmSubmit = async () => {
    ...
    const firstImage = formData.evidence.find(
      (ev) => ev.type === 'image' && ev.file
    );
    ...
    if (firstImage?.file) {
      try {
        ...
        console.warn(
          'Could not read evidence image for AI analysis:',
          err
        );
      }
    }
    ...
    const payload: CitizenProblemPayload = {
      ...
      evidence: formData.evidence.map((ev) => ({
        ...
        previewUrl: ev.previewUrl,
        base64:
          ev.type === 'image' && ev.id === firstImage?.id
            ? firstImageBase64
            : undefined,
      })),
      metadata: {
    ...
    const primaryUrl =
      `${configuredApiBase}/citizen/problems/confirm`;
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 400) {
          setSubmitError({
            ...
            message:
              data?.message ||
              'The server rejected the intake submission due to missing or invalid fields.',
            details: data?.errors,
          });
        } else {
          setSubmitError({
            ...
            message:
              data?.message ||
              'The intake service encountered an unexpected error.',
          });
        }
      ...
      const receipt: ProblemIntakeReceipt = {
        ...
        message:
          data?.message ||
          'Problem intake received successfully and queued for AI agent processing.',
      ...
      setSubmitError({
        ...
        message:
          'Could not connect to the CivicSolve intake backend endpoint. Please ensure the server is active.',
        details: [
          networkErr?.message ||
          'Request failed or was rejected by network policy.'
        ],
      });
    ...
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center space-y-4">
        ...
        <p className="text-sm text-red-800">
          {submitError?.message ||
            'The AI service did not return a result.'}
        </p>
  ...
  if (
    (stepState === 'preview' || stepState === 'submitting') &&
    aiResult
  ) {
    return (
        ...
        {aiCompletionPrompt && (
          ...
          >
            ...
            <p className="mt-1 text-sm text-blue-900">
              AI-generated details are ready. Would you like to use them
              in this report?
            </p>
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="btn-primary"
                onClick={() => {
                  setAiResult(aiCompletionPrompt);
                  setAiCompletionPrompt(null);
                }}
              >
                Yes, update details
              ...
              <button
                type="button"
                className="btn-secondary"
                onClick={() => setAiCompletionPrompt(null)}
              >
                No, keep my manual details
              </button>
            </div>
          </div>
        )}
      ...
      <div className="py-16 px-4 text-center bg-white border border-slate-200 rounded-2xl shadow-xs space-y-4">
        ...
        <div>
          <h3 className="text-base font-bold text-slate-900">
            {stepState === 'analyzing'
              ? 'Generating a reviewable AI draft'
              : t('translatingNotice')}
          </h3>

          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            {stepState === 'analyzing'
              ? 'Classifying the problem and checking the public registry without saving anything yet.'
              : 'Preparing an English translation preview for the municipal AI system while preserving your original words verbatim.'}
          </p>
        </div>
      </div>
    );
    ...
    >
      ...
      <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          <span>
            You will review your original description and its English
            translation before final submission.
          </span>
        </div>
